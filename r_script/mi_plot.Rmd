...
ent_mi <- read.csv("../results/results_ent_mi.csv", header=TRUE) %>%
  ...
  filter(model != "without_LLM_3",
         !language %in% c("Kor", "Sr")) %>%
  mutate(model = if_else(model == "without_LLM_1", "C-KDE-all", model),
         model = if_else(model == "without_LLM_2", "C-KDE-split", model)) %>%
...
df_entropy_combined %>%
  ...
  geom_segment(data = df_mi_subset,
               ...
               linetype = "solid", alpha = 0, linewidth = 0.7) +  # informative: alpha shows vertical line
...
mi_subwords_mgpt <- read.csv("../results/mi_subword_tokenization_relation.csv", header=TRUE) %>% 
  ...
  mutate(subword_filter = factor(subword_filter, levels = c("prop_mis_0", "prop_mis_1", "prop_mis_2", "prop_mis_3"), labels = c("All", "<= 3 subwords", "<= 2 subwords", "= 1 subword"))) %>%
